...
class GlueDataSource(DataSource):
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        # 지표에서 제외할 잡 (정확한 이름 또는 glob 패턴, 예: "test-*"). 제외분은 조회도 안 함.
        self._exclude_jobs = cfg.get("exclude_jobs") or []
        # fetch 시 실제로 제외된 잡 이름 (화면 표시용)
        self.excluded_jobs: list[str] = []

    def _is_excluded(self, name: str) -> bool:
        return any(fnmatch.fnmatch(name, pat) for pat in self._exclude_jobs)
    ...
    def fetch_runs(self) -> list[PipelineRun]:
        ...
        all_names = self._job_names or self._list_job_names(client)
        self.excluded_jobs = sorted(j for j in all_names if self._is_excluded(j))
        excluded = set(self.excluded_jobs)
        job_names = [j for j in all_names if j not in excluded]
